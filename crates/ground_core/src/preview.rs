...
use crate::target_look::{
    render_target_look_scene, target_look_pixel_to_cell, TargetLookRenderOptions,
};
...
pub enum PreviewMode {
    ...
    /// Diagnostic faux-perspective terrain: screen-aligned cells whose sprite
    ...
    AngledTerrain,
    /// Diagnostic software 2.5D visualization pass: top tiles are displaced upward by height,
...
pub fn render_terrain_preview(
    ...
    if mode == PreviewMode::PerspectiveSpriteScene {
        let target_options = TargetLookRenderOptions {
            show_route: options.show_projected_route,
            show_markers: options.show_scene_markers,
            show_debug: options.show_feature_overlay,
